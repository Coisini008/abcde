...
# 🔒 1. LINE 通知 Token 配置
# ==========================================
LINE_TOKEN = "70eKuBuGrO0BwWwSgDxwMPL/lN/C5WYk0LruRXR4PVYmfByGvsDfTN5zdOprzD2Xgq48kC6VD8UKkE8nLmw/9Esgz/1HgXPG4PsGR7u13BYacmiSmWODcUB77BE6BF3wKxve1DsniKC6M0DxIvUpNAdB04t89/1O/w1cDnyilFU="
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        if current_state == ST_IDLE:
            ...
            if movement_std > 5.0 and abs(jaw_velocity) > 4.0:
            ...
            elif (current_time - state_start_time) > CONFIG["MAX_CHEW_TIME"]:
        ...
        # 🎯 把 Patient 的高度從 45 改到 240，直接丟到最下方！
        cv2.putText(frame, f"Patient: {CONFIG.get('name', '未知個案')}", (20, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                    (255, 200, 100), 2)

        # 其他不變
        cv2.putText(frame, f"STATE: {current_state}", (20, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
        cv2.putText(frame, f"MAR (Mouth Open): {mar:.2f}", (20, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        cv2.putText(frame, f"Jaw Movement Std: {movement_std:.2f}", (20, 165), cv2.FONT_HERSHEY_SIMPLEX, 0.6,(255, 255, 255), 1)
        cv2.putText(frame, f"Chew Count: {int(chew_count)}", (20, 205), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 255, 100),2)

    if current_state == ST_SWALLOW:
            cv2.putText(frame, "SWALLOWING...", (20, h - 30), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
    else:
        cv2.putText(frame, "NO FACE DETECTED", (20, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
